	...
	public static void main(String[] args) throws IOException {
		...
		while(true) { // 프로그램 무한루프 
			...
			if( ch == -1 ) { // 만약에 -1번을 입력했으면 제품 등록 기능 
				...
				System.out.println(재고관리[ch-1].split(",")[0]+"를선택했습니다.");
				String name =  재고관리[ch-1].split(",")[0];
				int stock = Integer.parseInt( 재고관리[ch-1].split(",")[1] );
				int basket = Integer.parseInt( 재고관리[ch-1].split(",")[2] );
				int price = Integer.parseInt( 재고관리[ch-1].split(",")[3] );
				
				
				if( stock>0) {stock--; basket++; System.out.println(name+"재품을 담았습니다.");}
				else {
