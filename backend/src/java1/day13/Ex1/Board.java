...
public class Board {
	...
	// 1. 필드	: 객체의 데이터 저장하는 곳
		...
		- 필드 종류
		  - 인스턴스 필드			: (힙)객체 생성시 객체 안에 저장되어 있는 필드 - 사용
		  - 정적 필드 			: (메소드)[static] 프로그램 실행시 우선할당 되고 객체없이 저장되어 있는 필드-사용
		 ...
		 - 필드 타입
		  ...
		  -필드 접근 권한
		  	-접근 제한자
		  	public				: 외부에서 해당 필드를 접근할수 있는 제한/권한
		  	private 				: 프로젝내 모든곳
	...
	// 2. 생성자	: 객체 초기화 할때 사용
		/*
		 	...
		 	-생성자 선언
		 		접근제한자 클래스명(타입 매게변수, 타입 매게변수)
		 		
		 	- 생성자 역활
		 		- 1. 객체 생성시 매게변수를 받아서 필드에 저장
		 		- 2. 유효성검사 할때 / 계산식이 필요한 값을 필드에 저장할때
		 		
		 	- 생성자 종류 [ 오버로딩 으로 인해 다양하게 가능]
		 		-오버로딩 : 매게변수의 개수,순서, 타입에 따라서 식별 가능한 것
		 	- 접근제한 private , public , default
		 	
		 	- 기타 키워드
		 		- this 			: 	(생략가능) 현재 클래스의 구성맴버 식별할때 사용
		 		- this				: 	외부로부터 들어온 매게변수명 과 현재 클래스내 필드명과 같을때 필드 식별할때
		 		- this()			: 	현재 클래스의 생성자 호출
	...
	// 3. 메소드	: 객체의 행동/동작 수행할때 사용
		/*
		 	- 메소드 종류
		 		- 인스턴스 메소드	: 객체 생성시 사용 가능한 메소 
		 		- 적정 메소드			: (static) 객체 없이 사용 가능한 메소드 
		 		
		 	- 메소드 선언
		 		접근제한자 리턴타입 메소드명(타입 매게변수 , 타입 매개변수){ }
		 		
		 		
		 	-접근 제한자
		 		public , private , default , protected
		 	
		 	- 리턴타입				: 메소드 종료시 호출했떤곳으로 반환해주는 값의 타입
		 ...
		 - 메소드 사용목적
		 	...
		 	- 2. 재활용 : 코드 줄이기 / 클린 코딩
		 	- 3. 매게변수 에 따른 서로 다른 실행 / 결과 반환할수 있다.
