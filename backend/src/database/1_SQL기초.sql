	...
	데이터베이스 : 데이터[자료] , 베이스[모임]
		...
		- SQL : DB 조작/관리/제어 할때 사용되는 언어/문법 (대소문자 구분X)
			...
			-테이블 용어 / 테이블 
				...
				|		|		|				|
                학번		학생명	전화번호 			주소		-
                1010	박상빈	010-4444-3333	안산		- 가로 = 행 = row = 레코드	-1번 레코드
                1011	이진형	010-1213-2123	수원		- 가로 = 행 = row = 레코드	-2번 레코드
...
/*
#예1 : 데이터베이스 생성(여러개의 테이블들이 저장되는 공간) 생성
create database sqldb1;
	#create 	: 생성한다.
    #database 	: 데이터베이스.
    #sqldb1 	: 이름정의 [ 아무거나 , 대소문자 구분x 키워드x]
    #;			: 명령어 끝마침.
		# 해당 명렁어 줄에 커서 위치한 상태에서 컨트롤 엔터(실행)
		# navigator 새로고침
...
drop database sqldb5;
    ...
    #database 	:데이터베이스
    #sqldb1		:DB명
#예5 : 만약에 데이터베이스 존재했을때 삭제 [ *만약에 데이터베이스 없을때 삭제명렁어 사용하면 오류발생]
drop database if exists sqldb1;
...
use test1;
/* ---------------- 테이블(표) 만들기 -------------- */
# 테이블(표) 데이터베이스 안에 저장[ * 무슨 데이터베이스 생성 할건지.... use database명 ; ]
#예1 : 테이블 생성
