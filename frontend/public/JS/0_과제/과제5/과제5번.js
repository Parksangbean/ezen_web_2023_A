...
document.body.addEventListener('keydown' , ( e )=>{ console.log('키 눌림');
	...
	if( e.keyCode == 37 ){
		...
		userbox.style.backgroundImage = `url(img/이동.png)`;	// 배경변경 // 이동 이미지 출력 
	}else if( e.keyCode == 65 ){ // a키 눌렀을때
		console.log(' 공격 ');
		userbox.style.backgroundImage = `url(img/공격.png)`;  // 공격 이미지 출력 
	}
	// 3. 현재 위치값을 대입 
...
document.body.addEventListener( 'keyup' , (e)=>{ 
	// 원래 기본 이미지 변경
	userbox.style.backgroundImage = `url(img/유저.png)`;
})
