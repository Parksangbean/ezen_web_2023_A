...
function onWrite(){ console.log('onWrite()함수')
	...
	// 날짜 모양으로 출력
	let now=new Date(); // 1. 현재 시스템(pc)의 날짜/시간 출력
	let year=now.getFullYear();			console.log('year :'+year)	//현재 연도
	let month =now.getMonth()+1;		console.log(month)	//현재 월
	let day = now.getDate();				console.log('day: '+day)
	let hour = now.getHours();			console.log(hour)
	let minute = now.getMinutes();		console.log(minute)
	let second = now.getSeconds();	console.log(second)
