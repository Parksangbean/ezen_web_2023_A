/*
	...
	- 연산자
		...
		3. 비교 연산자 :
				>초과/크다 <미만/작다
				>=이상/크거나같다 <== 이하/작거나같다
				==[데이터 비교] === 같다[데이터형.데0이터 비교]
				!= 같지않다/아니다  !==같지않다/아니다
		
		
		
		4. 관계 연산자 : 주로 비교연산자 2개이상일때 사용 , true  혹은 false 
				&& 이면서 이고 모두 그리고 and		[모두 참이면  true ]
				|| 이거나 거나 또는  하나라도 or 조건 [하나라도 참true ]
				! 반대 부정:not 조건			!true=> / lfalse => true
		
		5. 대입연산자  :
				     = 대입 [오른쪽 데이터를 왼쪽 대입]
			     += [ 오른쪽 데이터를 왼쪽에 더한후에 대입]
			     	1.변수 = 10
		...
		6. 증감연산자  :
				++ 1증가[ 선위증가 : ++변수 /후위증가 : 변수++]
				--   1감소[ 선위감소 : --변수 / 후ㅇ
		7. 삼함연산자  : 항 3개 존재하는 연산자 , 참true or 거짓false 출력  
				조건 ? 참true : 거짓false
				- 만약에 조건이  true이면 참 실행
...
let 상티 = confirm('삭제하시겠습니까?')		// confirm( ) :확인(true) or 취소(flase) 버튼 존재하는 메세지 함수
